...
logger = launch.logging.get_logger('example_logger')

def generate_launch_description():
    ld = LaunchDescription()
    package_name = "turtlebot3_multi_robot"

    # [INFO] [1734059913.510322405] [ros_subscriber_node]: Received tb2 amcl_pose x: 8.830634839403299, y: 1.0622493509319912
    # [INFO] [1734059898.267367716] [ros_subscriber_node]: Received tb1 amcl_pose x: -4.409619070608956, y: 0.965375600186779
    ...
    # urdf = os.path.join(get_package_share_directory('turtlebot3_multi_robot'), 'urdf', 'robot_5.xacro')
    ...
    map_file = 'map_final.yaml'
    get_map_yaml = os.path.join(cur_directory, 'src', 'multi_gun_robot', 'map', map_file)
    map_server=Node(
        ...
        #              },],
        parameters=[{'yaml_filename': get_map_yaml}],
    ...
    for robot in robots:
        ...
        if last_action is None:
            ...
            ld.add_action(robot5_state_publisher)
            ...
            spawn_turtlebot3_event = RegisterEventHandler(
                event_handler=OnProcessExit(
                    ...
                    on_exit=[spawn_robot5, robot5_state_publisher, bringup_cmd],
                )
            )
